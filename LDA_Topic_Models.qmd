...
First, we read the data on we created in the last notebook, with info on the papers and their metadata (authors, publication year, etc.). Then, we transform the abstract texts into a tidy format, clean them up by removing common stop words and numbers, and ensure consistency by converting plural nouns into their singular forms. Next, these texts are transformed back into the wide format (i.e., one document per row).
...
Now we can start training our topic models `r emo::ji("hooray")` `r emo::ji("hooray")`. We need to train a series of models with different numbers of topics and then choose one with highest topic coherence. Here, we try 13 number of topics from 10 to 130.

```{r, eval=F}
readRDS("many_models_lda.rds") -> many_models_lda
# run topic models with k from 10 to 130
